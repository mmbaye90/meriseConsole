...
public class Main {
	...
	public static void listeDesReservations() {
		System.out.println(
				"""
						...
						""");
		// remplacement des clefs étrangeres par un de ses propriétés(id-ch par n°ch
		// id_client par son prénom)
		System.out.print(" idR\tdateDeb\t\tdateF  nbP   n°ch    Pcl");
		System.out.println();
		System.out.println("-------------------------------------------------");

		// utilisation de la méthode foreach(boucle) sur l'arraylist retourné par la
		// méthode getAll
		// de l'objet ReservDao,instancié directement avec le mot clef NEW pour afficher
		// la liste de réservation
		new ReservDao().getAll().forEach((res) -> {
			System.out.println("| " + res.getId_reserv() + " | " + res.getDateDebRes()
					+ " | " +
					res.getDateFinR() + " | " + res.getNbPerso() + " | "
					+
			// instanciation directe de l'objet Chambre pour faire appel à la méthode
			// getByid
			// en lui passant la clef etrangere enfin d'acceder aux propriétés de
					new ChambreDao().getById(res.getId_ch()).getNumCh()
					+
			// utilisation de la methode substring(extraire 0/3 caract) et de concat(...)
	...
	public static void listeDesClients() {
		...
		// utilisation de la méthode foreach(boucle) sur l'arraylist retourné par la
		// méthode getAll
		// de l'objet ClientDao,instancié directement avec le mot clef NEW pour afficher
		// la liste des clients
		System.out.println("| id |prenomC|adC    | ageC | ville | mailC    | sexeC | telC           |pays | nomC   |");
		System.out.println("----------------------------------------------------------------------------------------");
		new ClientDao().getAll().forEach((c) -> {
			System.out.println("| " + c.getId_client() + "  |" + c.getPrenomC().substring(0, 3).concat("...") + " |"
					+ c.getAdC().substring(0, 3).concat("...") + " | " + c.getAgeC() + "   | "
					+ c.getVilleC().substring(0, 4) + "  |" + c.getMailC().substring(0, 9) + " |   " + c.getSexeC()
					+ "   | " + c.getTelC() + " | " + c.getPaysC().substring(0, 2) + "  | "
					+ c.getNomC().substring(0, 4).concat("...") + "|");
			System.out.println(
					"----------------------------------------------------------------------------------------");
		});
	}
	...
	public static void listeDesChambres() {
		...
		// utilisation de la méthode foreach(boucle) sur l'arraylist retourné par la
		// méthode getAll
		// de l'objet ChambreDao,instancié directement avec le mot clef NEW pour
		...
		new ChambreDao().getAll().forEach((c) -> {
			System.out.println("| " + c.getId_ch() + "  |  " + c.getNumCh() + " |  " + c.getNbLitChSimp() + "  | "
					+ c.getNbLitChDoub() + "   | " + c.getSupCh() + "  | " + c.getSalleBainP() + " |" + c.getTvCh()
					+ " | " + c.getBalconCh() + " | " + c.getRefrigerCh() + " | " + c.getBaignCh() + " | "
					+ c.getInsonoCh() + " |" + c.getPrixNtCh() + "  |" +
			// appel de la methode getById()de la classe Hotel en lui passant en argument la
			// propriété
	...
	public static void listeLesPaiements() {
		...
		new PaiementDao().getAll().forEach((p) -> {
			...
			// ajout des propriétés de l'objet ReservDao dans l'objet Paiement pour
			// affichage
					new ReservDao().getById(p.getId_p()).getDateDebRes()
					+ "   | " + new ReservDao().getById(p.getId_p()).getDateFinR()
					+ "   | " + p.getMethP().substring(0, 2).concat("...") + "  |" + p.getMontP() + "  | "
					+ p.getDateP() + " |");
			System.out.println("--------------------------------------------------------------------");
		});
	}
	...
	public static void listerLesHotels() {
		System.out.println(
				"""
						...
						""");
		System.out.println(
				"| id | nom | adresse | ville |description |park |wifi |HrArr |HrDep |pisc |navt |animal | catH  | nomS |");
		System.out.println(
				"------------------------------------------------------------------------------------------------------");
		new HotelDao().getAll().forEach((h) -> {
			// Utilisation de switch case pour gérer l'affichage des étoiles selon la valeur
			// de la catégorie
			switch (h.getCatH()) {
				case 1:
					System.out.println("| " + h.getId_hotel() + "  |" + h.getNomH().substring(0, 2).concat("...") + "|"
							+ h.getAdresseH().substring(0, 5).concat("...") + " |"
							+ h.getVilleH().substring(0, 4).concat("...") + "|"
							+ h.getDescriptionH().substring(0, 9).concat("...") + "|" + h.getParkingH() + "|"
							+ h.getWifiH() + " | " + h.getHeureAaH() + "   | " + h.getHeureDepH() + "    | "
							+ h.getPiscineH() + "  | " + h.getNavetteH() + "  | " + h.getPresenceAniH().substring(0, 3)
							+ "  |" + "*".toUpperCase() + " |"
				...
				default:
					System.out.println("| " + h.getId_hotel() + "  |  " + h.getNomH() + " |  " + h.getAdresseH()
							+ "  | " + h.getVilleH() + "   | " + h.getDescriptionH() + "  | " + h.getParkingH() + " |"
							+ h.getWifiH() + " | " + h.getHeureAaH() + " | " + h.getHeureDepH() + " | "
							+ h.getPiscineH() + " | " + h.getNavetteH() + " |" + h.getPresenceAniH() + "  |"
							+ h.getCatH() + " |"
	...
	public static void listeLesSocietes() {
		...
		new SocieteDao().getAll().forEach((s) -> {
			System.out.println("| " + s.getId_soc() + "      |" + s.getNumSiret() + "  |" + s.getNom() + "   |"
					+ s.getAdresseS().substring(0, 5).concat("...")
					+ "|");
			System.out.println("---------------------------------------------------------");
	...
	public static void ajouterUnClient() {
		System.out.println(
				"""
						...
						""");
		// je déclare un boolean que j'initialise à false dans le try pour éxécuter ce
		// dernier
		// au moins une fois.Il me permet aussi de reexecuter ce bloc si l'utilisateur
		// entre un type
		...
		do {
			try {
				...
				// j'utilise la méthode equalsIgnoreCase() pour comparer la saisie de
				// l'utilisateur
				// à un format donné; Je redemande à l'uti tant qu'il ne saisie pas le bon
				// format
				do {
	...
	public static void ajouterUneChambre() {
		...
		do {
			try {
				...
				// j'utilise la boucle do while pour redemander à l'utili de resaisir tant que
				// la
				...
				// redemande à l'uti de resaisir tant que le format saisi n'est pas conforme à
				// celui
	...
	public static void effectuerUnPaiement() {
		...
		do {
			try {
				...
				// je récupére la date de début réservation en cours dans une var date1 de type
				// Str
				String date1 = new ReservDao().getById(idR).getDateDebRes();

				// je récupére la date de fin réservation en cours dans une var date2 de type
				// Str
				String date2 = new ReservDao().getById(idR).getDateFinR();

				// je récupére l'ID de la chambre correspondante à la reservation choisie par
				// l'utilisateur
				// avec la méthode getIdChambreByReser() de l'objet Reservation que je stocke
				// dans une var de type int
				int idch = new ReservDao().getIdChambreByReser(idR);

				// je récupére le prix de la chambre correspondante à la réservation choisie que
				// je stocke dans une var de type float
				float priUCh = new ChambreDao().getById(idch).getPrixNtCh();

				// je calcule le nombre de jour passé à l'hotel avec la méthode nbJourRes() et
				// le stocke
	...
	public static void ajouterUnHotel() {
		...
		do {
			try {
				...
				// ========================== Vérification si hr arr < ou > à hr dép avnt
				// insertion ds la BD ===================
				...
				// affichage des ID de la société ainsi que leur noms enfin de ne pas ajouter un
				// hotel sans société
				new SocieteDao().getAll().forEach((s) -> {
				...
				do {
					System.out.println("Saisir un ID parmis les ID proposés ci-dessus");
					idS = clavier.nextInt();
				} while (new HotelDao().getById(idS) == null);// redemande tant que l'ID de la société n'a pas été
																// selectionné
	...
	public static void ajouterUneSociete() {
		...
		// instanciation puis sette les val avec un constructeur avec parametre de
		// l'objet Societe
			...
			try {
				...
				try {
					...
					if (date1.before(date2)) {
						...
						System.out.println(
								"Date dé&but de réservation doit etre inferieure \nà la date de fin de reservation");
						effectuerUneReservation();
					}
	...
	public static void supprimerUnClient() {
		...
		do {
			// je l'initialise à false pour executer le bloc try au moins une fois et/ou si
			// la saie de l'uti ne corr
			err = false;
			try {
				...
				System.out.println(
						"ATTENTION les réservations du client seront aussi supp : 1(CONF)/0(ANN)".toUpperCase());
	...
	public static void supprimerUneChambre() {
		...
		do {
			...
			try {
				...
				System.out.println("ATTENTION les réservations liées à la chambre seront aussi supp : 1(CONF)/0(ANN)"
						.toUpperCase());
	...
	public static void supprimerUneReservation() {
		...
		do {
			...
			try {
				...
				System.out.println(
						"/!\\ les paiements liés à la réservation seront aussi supp : 1(CONF)/0(ANN)".toUpperCase());
	...
	public static void supprimerUnHotel() {
		...
		do {
			...
			try {
				...
				System.out
						.println("/!\\ les chambres liés à l'hotel' seront aussi supp : 1(CONF)/0(ANN)".toUpperCase());
	...
	public static void supprimerUneSociete() {
		...
		do {
			...
			try {
				...
				System.out
						.println("/!\\ les hotels liés à la societé seront aussi supp : 1(CONF)/0(ANN)".toUpperCase());
	...
	public static void modifierUneReservation() {
		...
		do {
			try {
				...
				String dateDeb;
				do {
					System.out.println("Saisir une date de début de reservation à mod au format (YYYY-MM-DD) "
							+ res.getDateDebRes());
					dateDeb = clavier.nextLine();
				...
				do {
					System.out.println(
							"Saisir une date de fin de reservation à mod format (YYYY-MM-DD) " + res.getDateFinR());
					dateFin = clavier.nextLine();
				} while (!isValidDate(dateFin));
				...
				int idClt;
				do {
					System.out.println("Veuillez choisir le client effectuant la réservation en tapant l'ID à mod "
							+ res.getId_client());
					idClt = clavier.nextInt();
	...
	public static void rechercherUnReservation() {
		...
		// j'instancie mesChambreDao/ClientDao/HotelDao pour pouvoir pointer sur leur
		// méthodes getById()
		...
		// j'affiche un message dans le cs où on a aucun résultat avec la méthode
		// isEmpty
		if (new ReservDao().recherche(mot).isEmpty()) {
			...
			new ReservDao().recherche(mot).forEach((res) -> {
				...
				// (req sql) et je les stockent dans des var de type int.ce qui me permet
				// d'avoir
	...
	public static void rechercherUnPaiement() {
		...
		if (new PaiementDao().recherche(mot).isEmpty()) {
			...
			new PaiementDao().recherche(mot).forEach((p) -> {
				...
				// je récupere l'id de la clef etrangere dans la table Paiement avec la meth
				// getIdResByPaiement
				int id_reserv = new PaiementDao().getIdResByPaiement(p.getId_p());

				// je récupere l'id de la clef etrangere dans la table Reservation avec la meth
				// getIdChambreByReser
				...
				// je remplace l'id_reserv par par deux de ses propriétés
				// getDateDebRes/getDateFinR
						new ReservDao().getById(p.getId_p()).getDateDebRes()
						+ "   | " + new ReservDao().getById(p.getId_p()).getDateFinR()
						+ "   | " + p.getMethP().substring(0, 2).concat("...") + "  |" + p.getMontP() + "    | "
						+ p.getDateP()
	...
	public static void rechercherUneChambre() {
		...
		// instanciation de l'hotel pour acceder à ses propriétés avec la méthode
		// getById
	...
	public static void rechercherUnHotel() {
		...
		new HotelDao().recherche(mot).forEach((h) -> {
			// récuperation de l'ID de la clef étrangere pour pointer vers la table Société
